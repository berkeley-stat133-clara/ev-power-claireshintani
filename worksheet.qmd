...
```{r}
str_detect(energy_sources, pattern = "Wind|Hydro")
```

b.  Extract the unit type (e.g., MWh, GWh, kWh) using str_extract(). Make sure your pattern handles extra spaces or hyphens.

```{r}
str_extract(energy_sources, pattern = ".Wh")
```

c.  Clean the names to keep only the energy source name, trimming whitespace and removing parentheses.

```{r}
energy_sources <- c("Solar", "Wind", "Coal", "Hydro")
# str_removeall(energy_sources, "\\s*\\(.*\\)")
```

d.  Create a clean tibble called energy_clean with two columns: source and unit.

```{r}
energy_clean <- tibble(source = energy_sources, unit = str_extract(energy_sources, pattern = ".Wh"))
energy_clean
```

e.  Discussion: Why might regex cleaning be important before performing joins or aggregations?\
    \
    Regex is important before performing joins or aggregations because we cannot join or aggregate across equivalent rows/columns if the data is not standardized, and thus cannot be matched.
...
```{r}
sales <- pivot_longer(sales, cols = c("Jan_Sales", "Feb_Sales", "Mar_Sales"), names_to = "Month", values_to = "Sales")
sales
```
...
```{r}
sales$Month <- str_sub(sales$Month, start = 1, end = 3)
sales
```
...
```{r}
pivot_wider(sales, names_from = "Month", values_from = "Sales")
```
